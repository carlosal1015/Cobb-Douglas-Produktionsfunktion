...
\begin{enumerate}
	...
	\item[Suficiencia] Reemplace en (6) $\bm{x}$ por $t\bm{x}$ y reagrupando esto como \[ \frac{tx_{1}F_{x_{1}}\left(t\bm{x}\right)+\cdots+tx_{n}F_{x_{n}}\left(t\bm{x}\right)}{F\left(t\bm{x}\right)}\leq m \] donde $t>1$. Esta ecuación puede ser reescrita como %\[ t\frac{d}{dt}\left(\ln F\left(t\bm{x}\right)\leq m\quad\text{o}\quad\frac{d}{dt}\left(\ln F\left(t\bm{x}\right)\leq\frac{m}{t}\right). \] Integrando luego la desigualdad desde $t=1$ o $t>1$ y omitiendo el signo $\ln$ obtenemos (5), completando la prueba de suficiencia.
\end{enumerate}
